...
void addEdge1(Graph graph, int v1, int v2, double trong_so){ //add edge in directed graph
    JRB node = jrb_find_int(graph.edges, v1);
    if (node == NULL){
        JRB list = make_jrb();
        jrb_insert_int(graph.edges, v1, new_jval_v(list)); 
        jrb_insert_int(list, v2, new_jval_d(trong_so));
    }
    else{
...
double dijkstra(Graph graph, int start, int stop, int* path, int* length){
    ...
    double distance[number]; // AAAAAAAAA
    ...
    Queue q = createQueue();
    q = Enqueue(q, start, distance[start]); // AAAAAAAAAA
    ...
    while(!IsQueueEmpty(q)){
        q = Dequeue(q, &deq);
        n = outdegree(graph, deq, output);
        visited[deq] = 1;
        for (int i = 0; i < n; i++){
            if (*(visited + output[i]) == 0){
                //printf("FLAG1\n");
                double temp = distance[deq] + getEdgeValue(graph, deq, output[i]);
    ...
    double dis = distance[stop];
    if (dis == INFINITIVE_VALUE) return dis;
    t[0] = stop;
    int a = 1;
    while(stop != start){
        stop = prev[stop];
...
int return_ID(Graph graph, char* name){
    JRB temp;
    jrb_traverse(temp, graph.vertices){
        if (strcmp(jval_s(temp->val), name) == 0) return jval_i(temp->key);
    }
    return -1;
}
...
{
    int row, columns;
    for (row=0; row<NumberVertex(graph); row++)   
    {
        for(columns=0; columns<NumberVertex(graph); columns++)
        { 
